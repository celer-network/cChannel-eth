...
const ERC827ExampleToken = artifacts.require('ERC827ExampleToken');
...
contract('GenericConditionalChannel', async accounts => {
  ...
  before(async () => {
    ...
    eRC827ExampleToken = await ERC827ExampleToken.deployed();
  ...
  contract('GenericConditionalChannel using ERC827', async () => {
    it('should return correct channel information when openChannel', async () => {
      const withdrawalTimeout = [1, 1];
      const settleTimeoutIncrement = 10000;
      const receipt = await instance.openChannel(
        ...
        withdrawalTimeout,
        settleTimeoutIncrement,
        eRC827ExampleToken.address,
        2  // 2 for TokenType.ERC827
      );
      ...
      assert.deepEqual(args.peers, peers);
      assert.equal(args.uintTokenType.toString(), '2'); // position '2' for ERC827
      assert.equal(args.tokenContract, eRC827ExampleToken.address);
    });
    ...
    it('should viewTokenContract and viewTokenType correctly', async () => {
      ...
      assert.equal(tokenContract, eRC827ExampleToken.address);
      assert.equal(tokenType.toString(), '2'); // position '2' for ERC827
    });
    ...
    it('should cooperativeWithdraw correctly', async () => {
      // approve first
      await eRC827ExampleToken.approve(instance.address, 100);
      const depositReceipt = await instance.depositERCToken(channelId, peers[0], 100);
    ...
    it('should fail to deposit (approve holds less tokens than deposit)', async () => {
      ...
      try {
        // approve first
        await eRC827ExampleToken.approve(instance.address, 50);
        await instance.depositERCToken(channelId, peers[0], 100);
      } catch (error) {
        err = error;
      }
